...
- **Lease epoch in runtime identity.** Include `leaseEpoch` in Kafka Streams `application.id` (or equivalent) so a new lease owner does not reuse committed state from a previous owner:
