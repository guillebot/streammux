...
/**
 * Fails fast on missing or blank Kafka/topic settings and logs the effective configuration at startup.
 */
public final class JobManagementRequiredSettingsEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    static {
        DeferredLog.replay(LOG, JobManagementRequiredSettingsEnvironmentPostProcessor.class);
    }
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        if (!problems.isEmpty()) {
            throw new IllegalStateException(
                "job-management-api cannot start: missing or blank required settings: " + String.join(", ", problems));
        }
        LOG.info(
            "job-management-api startup: kafka.bootstrap.servers="
